...
define variable reshape-callback :: <function> =
...
define variable display-callback :: <function> = callback-method () => ();
...
define variable mouse-callback :: <function> =
...
define variable passive-motion :: <function> =
...
define variable motion-callback :: <function> =
 callback-method (x :: <integer>, y :: <integer>)
 => ();
  let origin = make (<point>, x: as(<double-float>, x), y: as(<double-float>, y));
...
define variable keyboard-callback :: <function> =
 callback-method (key :: <integer>, x :: <integer>, y :: <integer>)
 => ();
  send-event (*screen*, make (<key-down-event>), key);
end;

define variable special-callback :: <function> =
 callback-method (key :: <integer>, x :: <integer>, y :: <integer>)
 => ();
  send-event (*screen*, make (<key-down-event>), key + #xFF);
end;
...
begin
  ...
  glutSpecialFunc (special-callback);
  ...
  call-out ("gluTessCallback", void:, ptr: *tess-object*.raw-value, int: 100100,
                                      ptr: c-expr (ptr: "glBegin"));
  call-out ("gluTessCallback", void:, ptr: *tess-object*.raw-value, int: 100101,
                                      ptr: c-expr (ptr: "glVertex3dv"));
  call-out ("gluTessCallback", void:, ptr: *tess-object*.raw-value, int: 100102,
                                      ptr: c-expr (ptr: "glEnd"));
