...
define constant $key-left = $GLUT-KEY-LEFT + #xFF;
define constant $key-right = $GLUT-KEY-RIGHT + #xFF;
define constant $key-up = $GLUT-KEY-UP + #xFF;
define constant $key-down = $GLUT-KEY-DOWN + #xFF;
...
define class <focus-event> (<event>)
end;

define class <focus-gained-event> (<focus-event>)
end;

define class <focus-lost-event> (<focus-event>)
end;
...
// send-event is called from the windows event callbacks with the topmost level
// shape, the *screen*, and is handled by classes in different ways depending on
// the type of shape and the type of event.

//define generic send-event (shape, event, data) => (result);

// <shape> handles converting between coordinate systems, locating the shape in
// the children hierarchy, and finally sending an on-mouse-event() to the shape.
...
define method send-event
    (shape :: <shape>, event :: <key-event>, key :: <integer>)
 => (result :: <shape>)
  on-key-event (shape, event, key);
  glutPostRedisplay ();
  shape;
end;

define method send-event
    (shape :: <shape>, event :: <focus-event>, data :: <object>)
 => (result :: <shape>)
  format-out ("here <<<\n");
  on-focus-event (shape, event, data);
  shape;
end;
  ...
  block (return)
    for (child in shape.children)
      ...
      if (contains-point? (child, new-origin / child.z-scale))
        return (send-event (child,
          make (object-class (event), origin: new-origin), button));
      end;
    end;
