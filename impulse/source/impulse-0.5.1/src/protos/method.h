...
namespace impulse {
 ...
 ///
    ...
    class Method : public Frame {
     ...
     // Constructors
		...
		Method( Value (T::*method)(Array&, Value) ) : _method( method ) { }
		Method( Value (T::*method)(Array&, Value), Frame* arg1proto ) : _method( method )
        {
 ...
 //
    ...
    Value Method<T>::eval( Array& args, Value self )
    {
        ...
        {
            if (margs.at( i ).getFrame()->proto() != _signature.at( i ).getFrame())
            {
                cout << "** Argument types do not match method signature" << endl;
                cout << "** " << margs.at( i ).inspect() << " " << _signature.at( i ).inspect() << endl;
